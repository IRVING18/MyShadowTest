...
object StepManager {
    ...
    fun execute(implClassName: String, stepImpl: StepImpl, stepTag: Int, delay: Long = DEFAULT_STEP_DELAY, data: Any? = null, begin: Boolean = false) {
        if (begin) isStop = false
        if (isStop) return
        StringBuilder().apply {
            append("\n>>>>>>>>>>>>execute>>>>>>>>>>>")
        ...
        stepCollector[implClassName] ?: registerObject(implClassName, stepImpl)
        stepCollector[implClassName]?.get(stepTag)?.execute(delay, data = data)
    }
     ...
     * 注册业务实现类
     */
    fun registerObject(implClassName: String, stepImpl: StepImpl) {
        StepCollector(implClassName).let {
            stepImpl.onImpl(it)
            stepCollector[implClassName] = it
        }
